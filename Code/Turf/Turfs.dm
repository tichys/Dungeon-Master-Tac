				...
				switch(icon_state)
					...
					if("Chasm")
						if(usr.density && !usr.Flying)
							...
							usr.z=4
							spawn(50)
								usr.Owner << "[usr] Manages to find his way to the surface"
								usr.z=1
							return
					else
						RestoreTemperature()
	...
	Items
		...
		Traps
							...
							if(M.Wagon == 0)
								...
								if (Result == "Enslave")
									for(var/mob/Monsters/A in src)
										if(A.Race == "Dragon")
											M.Owner << "[A.name] cant even fit in this cage.. you possible exploiter!...report it.."
											del A
											return
								...
								if (Result == "Capture") for(var/mob/Monsters/C in oview(1,M)) if(!HasPersonIn&&!C.Wagon)
									if(C.Race == "Dragon")
										M << "Are you crazy [C.name] is far to large to fit in that cage!"
										return
									if(!M.CanBeCaged)
										switch(C.Race)
											if("Gremlin","SandKing") usr << "[C.Race]s cannot be caged"
												...
												if(CAN)
													...
													if(!Escape)
														...
														C.PatrolPoint2 = null
														C.Deselect()
														var/F = C.Owner //Done this way to avoid a link between the owner and the salvers saves.
														C.cageoldowner = "[F]"
														C.Owner = "FREE" //Not Needed.
														C.BleedType = null
								...
								if (Result == "Tame")
									for(var/mob/Monsters/C in src)
										if(C.Race == "Dragon")
											M.Owner << "[C.name] cant even fit in this cage.. you possible exploiter!...report it.."
											del C
											return
										C.loc = M.loc
									...
									for(var/mob/Monsters/C in src)
										...
										for(var/mob/G in world)
											if(G.name == "[F]")
												...
												C.Owner = G
												//C.ChangeOwnership(G)
												//G.UnitList-=C //Only needed if using the ChangeOwnership() proc
												//world << "it worked" //Keeping for historical purposes lol.
										C.InHole = 0
										C.Caged = 0
				Content3 = "Cage"
				GoldCage
			...
			PitTrap  //go here
				...
				Click(location,control,params)
					...
					if(P["right"])
						for(var/mob/Monsters/M in usr.Selected)
							view() << "[M] is disarming the trap."
							sleep(50)
							view() << "[M] disarmed the trap."
							del(src)
							return
					if(P["left"])
						for(var/mob/Monsters/M in usr.Selected)
							if(src.Owner == M.Owner)
								view() << "[M] is covering up the trap."
								sleep(50)
								view() << "[M] has hidden the trap."
								src.icon = null
								src.Hole = 1
								return
			BST
		...
		Shafts
							...
							if(M.Wagon == 0)
								...
								if (Result == "Cast")
									...
									if (src.Content2 == "Teleportation")
										M.Owner << "Teleportation has been removed."
										return
										/*var/X = M.icon
			...
			LockPick
				...
				weight = 5
				suffix = null
				desc = "This is a lockpick, it can be used to open locked doors and chests, it will break if you success to lockpick an item"
				DblClick()
		...
		Machinery
			...
			Parts
				...
				AutomatonComplete
					...
					weight = 10
					suffix = null
					desc = "This is a completed Automaton."
					DblClick()
						for(var/mob/Monsters/M in usr.Selected)
							if(src in view(1,M))
								M.destination = null
		...
		Materials
			...
			Gulver
				...
				weight = 5
				suffix = null
				desc = "This is a piece of extremly conductive and soft alloy."
				DblClick()
					for(var/mob/Monsters/M in usr.Selected)
						if(src in view(1,M))
							M.destination = null
							...
							if(M.Wagon == 0)
								...
								if (Result == "Transmute Into Gold")
									if(src.suffix == "(Carrying)")
										...
										M.weight += B.weight
										M.weight -= src.weight
										view(M) << "[M] uses their force of will to seprate the gold and silver in the alloy."
										del src
										return
		Ingots
			New()
						...
						if(src in M)
							if(M)
								...
								if (Result == "Plant")
									if(src.suffix == "(Carrying)")
										for(var/turf/T in view(0,M))
											for(var/obj/Grower/G in M.loc)
										//		if(S != src)
												usr << "Cant plant too many seeds in one area!"
												return

											if(T.icon_state == "Grass" || T.icon_state == "Desert" || T.Content == "Marsh" || T.icon_state == "Snow")
												var/obj/Grower/Tree/G = new(T,usr)
												spawn() G.TreeGrow()
												M.weight -= src.weight
												spawn(1) del src
												return
						...
						for(var/mob/Monsters/M in usr.Selected)
							if(src.Placed == 1)
								...
								if (Result == "Cancel")
									return
								if (Result == "Attack")
									usr << "[M] begins attacking [src] this may take a minute."
									spawn(200-M.Strength)
										Success = prob(0.95*M.Strength)
										if(Success == 1)
											usr << "[M] destroys [src]."
											del src
											return
										else
											usr << "[M] successs to destroy [src] try again."
											return
								if(Result == "Unplace")
									if(src.Placed == 1)
										if(M.Owner == src.Owner)
											...
											src.opacity = 0
											src.Door = 0
											usr << "[M] uproots [src] it can now be picked up."
									//M.destination = src
							if(src in view(1,M))
								M.destination = null
								if(src.Placed == 0)
