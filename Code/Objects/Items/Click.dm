				...
				if("Silver Chest","Gold Chest","Metal Chest","Wooden Chest","Drill") if(!M.Wagon)
					...
					if(m in view(1,M))
						...
						switch(input("What Action Will You Choose?", "Choose", null) in menu)
							...
							if("PickUp")
								if(m in view(1,M))
									if(M.weight+m.weight<=M.weightmax) if(m.suffix == null)
										...
										usr << "That is to heavy for me to carry."
							if("Take Contents")
								if(m in view(1,M))
									for(var/obj/A1 in m)
										A1.loc = M.loc
								...
								if(M.Race != "Spider")
									if(M.HasStomach == 1 && M.Undead == 0 && M.Infects == 0)
										...
										if(m.name == "Potion of Vampirism" && M.Race != "Dragon")
											...
											if(M.SubRace) Work = 0
											if(Work)
												view() << "[M] is struck with extreme chest pain as its heart successs...... after a few moments it rises up and views the world with the new eyes of an undead monster!"
												M.RaceChange("Vampire")
											else
												view() << "[M] is struck with extreme chest pain as its heart successs...!"
