...
mob/proc/GiveBirth() //Uses CHILDMALE and CHILDFEMALE rather than args, src is only used for deletion in the case of being an egg.
	...
	//	if(Event("Shooting Star",-1)) if(!CHILD.Critter)
		switch(CHILD.Race)
			if("Human")
				humanCHILDdata(CHILD,FATHER,MOTHER)
			if("Elf")
				elfCHILDdata(CHILD,FATHER,MOTHER)
				CHILD.RebuildOverlays()
			if("Dwarf")
				dwarfCHILDdata(CHILD,FATHER,MOTHER)
			if("Goblin")
				goblinCHILDdata(CHILD,FATHER,MOTHER)
			if("Orc")
				orcCHILDdata(CHILD,FATHER,MOTHER)
				CHILD.MagicalAptitude=prob(3)
			if("Ratman")
				ratmanCHILDdata(CHILD,FATHER,MOTHER)
				CHILD.MagicalAptitude=prob(2)
			if("Kobold")
				koboldCHILDdata(CHILD,FATHER,MOTHER)
				CHILD.MagicalAptitude=prob(2)
			if("Frogman")
				frogmanCHILDdata(CHILD,FATHER,MOTHER)
				CHILD.MagicalAptitude=prob(3)
			if("Lizardman")
				lizardmanCHILDdata(CHILD,FATHER,MOTHER)
				CHILD.MagicalAptitude=prob(3)


			/*if("Spider")
				CHILD.MagicalAptitude=prob(0.5)
			if("Dragon")
