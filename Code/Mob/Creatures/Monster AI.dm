...
mob/proc/walking(src,B)
	if(get_dist(src,B) > 1)
		walk_to(src, B, 1,4)
	else
		walk_towards(src,B,4)
...
mob/Monsters/proc/Guarding(RANGE = 5,RATE = 10,AUTO = 0)
	if(!Critter)
		if(StopDouble("Guard"))
			HomeLoc = loc
			spawn()
		...
		if(!InHole && CanWalk)
			if(destination)
				if(ismob(destination) || destination == HomeLoc)
					if(destination in range(6,src))
						walking(src,destination)
					else
						if(destination==HomeLoc)
							walking(src,destination)
...
/mob/Monsters/proc/GuardWalk(LINKED = 0)
...
/mob/Monsters/proc/LocateTarget(RANGE = 5)
	for(var/mob/Monsters/M in oview(RANGE,src))
		...
		if(!InHole && CanWalk)
			...
			if(ismob(M.Owner))
				switch(Owner)
					if("{NPC Alliance}")
						if(!M.Undead && !M.Morphed && M.SubRace != "HalfDemon" && !M.Werepowers)
							Gunna = 0
					if("{NPC Undead}")
						if(M.Undead || M.Race == Race)
							Gunna = 0
					else if(M.Race == Race)
						Gunna = 0
			...
			if(ismob(Owner)&&ismob(M.Owner))
				...
				if(OWNERB.name in OWNERA.AllyList)
					Gunna=0
				if(OWNERB.Faction == OWNERA.Faction)
					Gunna=0
			if(ismob(M.destination))
				var/mob/Monsters/D = M.destination
				if(D.Owner == Owner)
					Gunna = 1
			if(M.Owner == Owner || M.CantKill)
				continue
			switch(M.Race)
				if("Plant")
					if(prob(99))
...
/mob/proc/FindGear()
	...
	for(var/obj/Items/I in view(1,src))
		...
		break
	if(OOO == 1)
		for(var/obj/Items/Equipment/E in view(1,src))
			...
			switch(E.ItemType)
				...
				if("Shield")
					if(!WearingShield)
						TryIt = 1
				else if(E.EquipmentType() == "Weapon")
					if(!HoldingWeapon)
						TryIt = 1
			if(TryIt)
				if(PickUpItem(E))
...
mob/Monsters/proc/Patrol(LOC1,LOC2)
	...
	PatrolPoint2=LOC2
	HomeLoc=PatrolPoint1
mob/Monsters/proc/Guarding(RANGE=20,RATE=10,AUTO=0) if(!Critter) if(StopDouble("Guard"))
	HomeLoc=loc
	spawn()
		spawn() while(HomeLoc)
			sleep(250)
...
mob/Monsters/proc/GuardWalk(LINKED=0) if(StopDouble("WalkTo") || LINKED) spawn()
	...
	while(CanBeSlaved)
		...
		if(!InHole && CanWalk)
			...
			if(destination) walking(src,destination)
	WalkTo(1)
mob/Monsters/proc/LocateTarget(RANGE=20) for(var/mob/Monsters/M in oview(RANGE,src))
	...
	if(Gunna)
		...
		break
			/*if(C in oview(5,M))
				if(C.IsWall==1)
				for(var/obj/Items/Equipment/Weapon/Picks/PickAxe/E in src)
					//src.EquipItem(E)
				step_to(src,C,1,4)
				return(C)
				break
			else
				for(var/obj/Items/Equipment/Weapon/E in src)
					src.EquipItem(E)
				step_to(src,M,1,4)
				return(M)
				break*/
...
/mob/proc/MakeHostile(NAME="Basic",TIME=6000)
	if("[NAME]" in Hostilities)
		return
	Hostilities += "[NAME]"
	if(TIME > 0)
		spawn(TIME) Hostilities -= "[NAME]"
	return(1)

mob/Monsters
	var/tmp/ReturnDelay = 400
	var/tmp/PatrolPoint1
	var/tmp/PatrolPoint2
	var/tmp/HomeLoc = null
	var/tmp/list/GuardRules = new

mob/Monsters
	var/list/Friendlies = new
...
mob/verb/Guard()
	...
	switch(input("What Action Will You Choose?", "Choose", null) in menu)
		...
		if("Check Rules")
			for(var/mob/Monsters/M in usr.Selected)
				var/RULES="[M]'s Rules:"
				for(var/R in M.GuardRules)
					RULES += " '[R]'"
				if(M.PatrolPoint1&&M.PatrolPoint2)
					RULES += " 'Patrol'"
				var/FRIENDLIES = "[M]'s Friendlies:"
				for(var/F in M.Friendlies)
					FRIENDLIES += " '[F]'"
				usr << "[RULES]<br>[FRIENDLIES]<br>"
		if("Add Patrol Rule")
			usr << "<b>Click tile to create a patrol route between that location and your unit's current location."
		...
		if("Add Agressive Rule")
			for(var/mob/Monsters/M in usr.Selected)
				if(!M.Wagon)
					if(!M.GuardRules.Find("Agressive"))
						M.GuardRules += "Agressive"
			usr << "<b>Agressive Rule added to selected units. They will now automatically attack any units they see while on guard."
		if("Set Friendly to...")
			var/FRIEND = input("Set a Owner to friendly for selected units, input including parentheses if adding a NPC Owner.","Add Friendly","{NPC Alliance}") as text|null
			if(FRIEND)
		...
		if("Clear Rules")
			...
			for(var/mob/Monsters/M in usr.Selected)
				...
				M.PatrolPoint2 = null
				M.GuardRules = list()
				if(M.destination == M.PatrolPoint1 || M.destination == M.PatrolPoint2)
					M.destination = null
				if(M.HomeLoc == M.PatrolPoint1 || M.HomeLoc == M.PatrolPoint2)
					M.HomeLoc = null
...
/mob/Monsters/proc/RandomWalk(LINKED = 0)
	if(StopDouble("WalkTo") || LINKED)
		if("Guard" in RunningProcs)
			GuardWalk(1)
			return
