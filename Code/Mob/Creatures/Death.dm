/*mob/proc/Death()
	...
	if(src.GoingToDie)
		switch(Race)
			if("Mole","CaveSpider","Deer")
				view(src) << "<p><b><h1><font color=yellow>[src] has died at age [Age].<p>"
			if("TowerCap","Plant")
				view(src) << "<p><b><h1><font color=yellow>[src] has withered.<p>"
			else
				view(src) << "<p><b><h1><font color=yellow>[src] has died at age [Age] due to [src.Killer] and died from [DeathType].<p>"
		if(src.Race == "Kobold")
			if(src.Level >= 20)
		...
		if(ismob(Owner)) if(!Critter) Legends = {"[Legends]<p>[src] - Died [Day],[Month],[Year] at age [Age], [name] had reached level [Level] before being killed by [Killer] and died from [DeathType]"}
		del(src)*/
/mob/proc/Death()
	if(key||CantKill||Wagon)
		return
	if(src.GoingToDie)
		if(src.Race == "Kobold")
			if(src.Level >= 20)
		...
		if(src.Killer && ismob(src.Killer))
			if(!Critter)
				var/mob/K = src.Killer
				if(!Undead)
					for(var/obj/Items/Equipment/Weapon/DemonicWeapons/SoulBlade/S in K)
				...
				if(Killer != src)
					...
					switch(Race)
						...
						if("Spider") EXPREWARD*=0.75
						if("Skeleton") EXPREWARD*=0.4
					for(var/mob/Monsters/M in view(K,6))
						if(M.Owner != src.Owner)
							if(M != K)
								if(M.Owner==K.Owner)
									M.GainEXP(EXPREWARD*0.1)
					K.GainEXP(EXPREWARD)
					if(ismob(K.Owner))
						if(K != src)
							K.Owner<<"[Killer] gained [EXPREWARD*ExperienceMultiplier] Experience for slaying [src]"
		...
		if(ismob(Owner))
			if(!Critter)
				Legends = {"[Legends]<p>[src] - Died [Date] at age [Age], [name] had reached level [Level] before being killed by [Killer] and died from [DeathType]"}
		del(src)
...
/mob/proc/TearSkin(mob/Body/BODY,Telling = 0)
	if(BODY.loc == src)
		if(!BODY.Skinned)
			BODY.Skinned = 1
			var/obj/Items/SKIN
			switch(BODY.Race)
				if("Fish","Spider","CaveSpider","Plant","DeathBeatle","Skeleton")
					...
					if(BODY.Black)
						SKIN.icon_state = "[icon_state]Black"
					if(BODY.Infects)
						SKIN.icon_state = "Zombie"
			if(BODY.CarryPartStats(SKIN))
				return(SKIN)
		else if(Telling)
			usr << "[BODY] has already been skinned."
