...
/*Admin/verb/mapSave() //the map save verb
	set category="Admin"
	SaveObjects()
Admin/verb/mspLoad() //the map load verb
	set category="Admin"
	LoadObjects()*/
mob/Topic(hr,h[],hs)
...
Admin/verb/ChangeDamageMultiplier(DAMAGE as num)
	set name = "Change damage multiplyer"
	set category = "Admin"
...
Admin/verb/ChangeExperienceMultiplier(EXPERIENCE as num)
	set name = "Change experience multiplayer"
	set category = "Admin"
...
Admin/verb/CheckPlayerGenders()
	set name = "Check player genders"
	set category="Admin"
	for(var/mob/M in Players2)
		usr << "[M] - [M.gender]"

Admin/verb/FakeLogout()
	set name = "Fake logout"
	set category="Admin"
...
Admin/verb/CheckRaces()
	set name = "Check races"
	set category="Admin"
	for(var/mob/M in world)
		if(M.key)
			usr << "--<font color = purple>[M] - Race:[M.Race]"
...
Admin/verb/SendSound(var/mob/person in Players2,filez as file)
	set name = "Send sounds"
	set hidden = 1
	person << sound(filez)

Admin/verb/SendSoundAll(filez as file)
	set name = "Send all sounds"
	set hidden = 1
	world << sound(filez)

Admin/verb/SoundOffAll()
	set name = "Turn all sounds off"
	world << sound(null)
...
Admin/verb/Announcement(msg as message)///Nifty window pop-up announcement
	...
	set category="Admin"
	var/html_doc="<head><title>Announcement</title></head><body bgcolor=#000000 text=#FFFF00><center><B><U><font color = red><font size=1><font face=Verdana>Announcement</U> from [usr]:</font><font face=Verdana><font color=COCOCO><br>[msg]"
...
Admin/verb/Summoner(mob/M in world)
	set hidden = 1
	//set name = "Summon mob"
	M.x = usr.x; M.y = usr.y; M.z = M.z; if(M.client) M.client.eye = M;
...
Admin/verb/ServerOptions()
	...
	switch(input("Select Option","Server Panel")as null|anything in list ("Reboot Server","Shutdown Server","Repop Server","Change Server Name","Change Server Status","Admin Log","Chat Log","Unitchat Log","Emote Log"))
		if("Reboot Server")
			...
			sleep(100)
			world.Reboot()
		if("Shutdown Server") if(usr.key == "Godsring")
...
mob/Developer/verb/RealPrivateMessage(var/mob/M in Players, msg as text)
	...
	for(var/mob/D in Players2)
		if(D != M && D != usr)
			if(D.key == "Godsring" || D.key == "Virgili555")
...
/proc/load_admin_ranks()
	...
	var/list/Lines = file2list("config/admins.txt")
		...
		D.associate(directory[ckey])	//find the client for a ckey if they are connected and associate them with the new admin datum
*/
proc/Log_bug(adminaction) //Define adminaction as a string then call this proc
...
var/Bugs = null//The var for the logging
