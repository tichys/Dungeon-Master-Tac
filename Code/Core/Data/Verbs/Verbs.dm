...
turf/proc/MeteorStrike()
	switch(rand(1,2))
		if(1)
			...
			src.Sky = 1
			if(prob(5))
				var/obj/Items/gems/Diamond/U = new
				U.loc=src
				U.Content = "Flawless"
				U.name = "Flawless Diamond"
			if(prob(10))
				var/obj/Items/gems/Ruby/U =new
				U.loc=src
				U.Content = "Flawless"
				U.name = "Flawless Ruby"
			if(prob(15))
				var/obj/Items/gems/Emerald/U = new
				U.loc=src
				U.Content = "Flawless"
				U.name = "Flawless Emerald"
		if(2)
			src.CanFish = 0
			src.Content3 = null
			src.name = "Mountain"
...
atom/proc/Smoke()
	...
	spawn(50)
		Smoke()
/*mob/verb/GetCoords() src << "/c [Topic_xyz()]<br>"
atom/proc/Topic_xyz(txt) return("<a href=?action=xyz;x=[x];y=[y];z=[z]>[txt?txt : "[x],[y],[z]"]</a>")
proc/Topic_xyz_alt(x,y,z,txt) return("<a href=?action=xyz;x=[x];y=[y];z=[z]>[txt?txt : "[x],[y],[z]"]</a>")
mob/proc/Topic_Locate(txt) return("<a href=?src=\ref[src];action=Locate>[txt?txt : name]</a>")
mob/Topic(href,href_list[]) switch(href_list["action"]) if("Locate") if(Owner==usr) usr.loc=loc
client/proc/Topic_SendMSG(txt) return("<a href=?who=[ckey];action=PM>[txt?txt : key]</a>")*/
mob/verb/GetCoords() src << "/c [Topic_xyz()]<br>"
atom/proc/Topic_xyz(txt) return("<a href=?action=xyz;x=[x];y=[y];z=[z]>[txt?txt : "[x],[y],[z]"]</a>")
proc/Topic_xyz_alt(x,y,z,txt) return("<a href=?action=xyz;x=[x];y=[y];z=[z]>[txt?txt : "[x],[y],[z]"]</a>")
mob/proc/Topic_Locate(txt) return("<a href=?src=\ref[src];action=Locate>[txt?txt : name]</a>")
mob/Topic(href,href_list[]) switch(href_list["action"]) if("Locate") if(Owner==usr) usr.loc=loc
client/proc/Topic_SendMSG(txt) return("<a href=?who=[ckey];action=PM>[txt?txt : key]</a>")
mob/proc/Topic_SendMSG(txt) return("<a href=?who=[key];action=PM>[txt?txt : key]</a>")
proc/TextFilter(txt,limit=300)
	if(length(txt)>limit) txt=copytext(txt,1,limit)
	txt=html_encode("[txt]")
	var/C = findtext(txt,"/c ")
	while(C)
		var/a=findtext(txt,",",C+3)
		var/b=findtext(txt,",",a+1)

		var/X=copytext(txt,C+3,a)
		var/Y=copytext(txt,a+1,b)
		var/Z=copytext(txt,b+1,b+2)

		if(length("[X]")>3||length("[Y]")>3||length("[Z]")>1) break
		if("[text2num(X)]"==X&&"[text2num(Y)]"==Y&&"[text2num(Z)]"==Z)
			txt="[copytext(txt,1,C)]<a href=?action=xyz;x=[X];y=[Y];z=[Z]>[X],[Y],[Z]</a>[copytext(txt,b+2)]"
			C = findtext(txt,"/c ")
		else break
	return(txt)
atom/proc/Fire()
...
mob/proc/Close()
	if(src.Up == 0)
		...
		src.Up = 0
		for(var/obj/HUD2/H in src.client.screen)
			del(H)
...
mob/verb/Test()
	...
	if(usr.Up == 0)
		...
		usr.Up = 0
		for(var/obj/HUD2/H in usr.client.screen)
			del(H)
...
mob
	proc
		Stations()//This is the proc that creates the OSI
			for(var/obj/HUD2/H in usr.client.screen)//Deletes any HUD on-screen
				del(H)
			usr.StationMenu()//Creates the OSI Background
		StationMenu()//Proc that creates the OSI Background
			for(var/mob/Monsters/M in usr.Selected)
				var/obj/H = new/obj/HUD2/Middle2(usr.client)
				var/obj/Smelt = new/obj/HUD2/ClickSmelt(usr.client)
				var/obj/Forge = new/obj/HUD2/ClickForge(usr.client)
				var/obj/Carp = new/obj/HUD2/ClickCarp(usr.client)
				var/obj/Potion = new/obj/HUD2/ClickPotion(usr.client)
				var/obj/Pos = new/obj/HUD2/ClickPos(usr.client)
				var/obj/Mas = new/obj/HUD2/ClickMas(usr.client)
				var/obj/Kit = new/obj/HUD2/ClickKit(usr.client)
				var/obj/Let = new/obj/HUD2/ClickLet(usr.client)
				var/obj/Gem = new/obj/HUD2/ClickGem(usr.client)
				var/obj/Bone = new/obj/HUD2/ClickBone(usr.client)
				var/obj/Melt = new/obj/HUD2/ClickMelt(usr.client)
...
mob
	proc
		...
		FarmMenu()//Proc that creates the OSI Background
			for(var/mob/Monsters/M in usr.Selected)
				var/obj/H = new/obj/HUD2/Middle2(usr.client)
				var/obj/O = new/obj/HUD2/TopLeft(usr.client)
				var/obj/A = new/obj/HUD2/TopRight(usr.client)
				var/obj/B = new/obj/HUD2/BottomLeft(usr.client)
				var/obj/C = new/obj/HUD2/BottomRight(usr.client)
				var/obj/D = new/obj/HUD2/TopMiddle(usr.client)
				var/obj/E = new/obj/HUD2/BottomMiddle(usr.client)
				var/obj/F = new/obj/HUD2/LeftMiddle(usr.client)
				var/obj/G = new/obj/HUD2/RightMiddle(usr.client)
				var/obj/Farm = new/obj/HUD2/ClickFarm(usr.client)
	...
	proc
		...
		TrapMenu()//Proc that creates the OSI Background
			for(var/mob/Monsters/M in usr.Selected)
				...
				var/obj/F = new/obj/HUD2/LeftMiddle(usr.client)
				var/obj/G = new/obj/HUD2/RightMiddle(usr.client)
				var/obj/Trap = new/obj/HUD2/ClickTraps(usr.client)
				var/obj/StoneTrap = new/obj/HUD2/ClickStoneFall(usr.client)
				var/obj/RibTrap = new/obj/HUD2/ClickRibTrap(usr.client)
				var/obj/BST = new/obj/HUD2/ClickBSTTrap(usr.client)
				H.screen_loc = "4,14 to 14,4"
				O.screen_loc = "4,14"
				A.screen_loc = "14,14"
				B.screen_loc = "4,4"
	...
	proc
		...
		FloorMenu()//Proc that creates the OSI Background
			for(var/mob/Monsters/M in usr.Selected)
				...
				var/obj/F = new/obj/HUD2/LeftMiddle(usr.client)
				var/obj/G = new/obj/HUD2/RightMiddle(usr.client)
				var/obj/WoodFloor = new/obj/HUD2/ClickWoodFloor(usr.client)
				var/obj/TribalFloor = new/obj/HUD2/ClickTribalFloor(usr.client)
				var/obj/StoneFloor = new/obj/HUD2/ClickStoneFloor(usr.client)
				var/obj/BoneFloor = new/obj/HUD2/ClickBoneFloor(usr.client)
				var/obj/DigChannel = new/obj/HUD2/ClickDigChannel(usr.client)
				var/obj/Dest = new/obj/HUD2/ClickDestroy(usr.client)
				H.screen_loc = "4,14 to 14,4"
				O.screen_loc = "4,14"
				A.screen_loc = "14,14"
				B.screen_loc = "4,4"
	...
	proc
		...
		BridgeMenu()//Proc that creates the OSI Background
			for(var/mob/Monsters/M in usr.Selected)
				...
				var/obj/F = new/obj/HUD2/LeftMiddle(usr.client)
				var/obj/G = new/obj/HUD2/RightMiddle(usr.client)
				var/obj/TribalBridge = new/obj/HUD2/ClickTribalBridge(usr.client)
				var/obj/StoneBridge = new/obj/HUD2/ClickStoneBridge(usr.client)
				var/obj/WoodBridge = new/obj/HUD2/ClickWoodBridge(usr.client)
				var/obj/BoneBridge = new/obj/HUD2/ClickBoneBridge(usr.client)
				H.screen_loc = "4,14 to 14,4"
				O.screen_loc = "4,14"
				A.screen_loc = "14,14"
				B.screen_loc = "4,4"
	...
	proc
		...
		WallMenu()//Proc that creates the OSI Background
			for(var/mob/Monsters/M in usr.Selected)
				...
				var/obj/F = new/obj/HUD2/LeftMiddle(usr.client)
				var/obj/G = new/obj/HUD2/RightMiddle(usr.client)
				var/obj/WoodWall = new/obj/HUD2/ClickWoodWall(usr.client)
				var/obj/ElfWall = new/obj/HUD2/ClickElfWall(usr.client)
				var/obj/TribalWall = new/obj/HUD2/ClickTribalWall(usr.client)
				var/obj/StoneWall = new/obj/HUD2/ClickStoneWall(usr.client)
				var/obj/BoneWall = new/obj/HUD2/ClickBoneWall(usr.client)
				var/obj/SecretWall = new/obj/HUD2/ClickSecretWall(usr.client)
				var/obj/Detail = new/obj/HUD2/ClickDetail(usr.client)
				var/obj/ArrowSlit = new/obj/HUD2/ClickArrowSlit(usr.client)
				H.screen_loc = "4,14 to 14,4"
				O.screen_loc = "4,14"
				A.screen_loc = "14,14"
				B.screen_loc = "4,4"
		...
		MindMenu()//Proc that creates the OSI Background
			for(var/mob/Monsters/M in usr.Selected)
				if(M.SubRace == "Illithid")
					...
					var/obj/F = new/obj/HUD2/LeftMiddle(usr.client)
					var/obj/G = new/obj/HUD2/RightMiddle(usr.client)
					var/obj/Wall = new/obj/HUD2/Forcefield(usr.client)
					var/obj/Telekinesis = new/obj/HUD2/Telekinesis(usr.client)
					var/obj/Mindcontrol = new/obj/HUD2/Mindcontrol(usr.client)
					var/obj/Pyrokinesis = new/obj/HUD2/Pyrokinesis(usr.client)
					var/obj/ReadMind = new/obj/HUD2/ReadMind(usr.client)
					var/obj/Warp = new/obj/HUD2/Warp(usr.client)
					//var/obj/Cacoon = new/obj/HUD2/Cacoon(usr.client)
					//var/obj/Br = new/obj/HUD2/B(usr.client)
					//var/obj/Dec = new/obj/HUD2/Dec(usr.client)
					var/obj/Scrying = new/obj/HUD2/Scrying(usr.client)
					H.screen_loc = "4,14 to 14,4"
					O.screen_loc = "4,14"
					A.screen_loc = "14,14"
					B.screen_loc = "4,4"
...
mob
	proc
		Building()
			for(var/obj/HUD2/H in usr.client.screen) del(H)
			usr.BuildingMenu()
		BuildingMenu()
			for(var/mob/Monsters/M in usr.Selected)
				if(M.density == 1)
					...
					/*if("Spider")
						...
						var/obj/F = new/obj/HUD2/LeftMiddle(usr.client)
						var/obj/G = new/obj/HUD2/RightMiddle(usr.client)
						var/obj/Wall = new/obj/HUD2/SilkWall(usr.client)
						var/obj/Stairs = new/obj/HUD2/Stairs(usr.client)
						var/obj/Trap = new/obj/HUD2/SpiderTrap(usr.client)
						var/obj/Post = new/obj/HUD2/Post(usr.client)
						var/obj/Cacoon = new/obj/HUD2/Cacoon(usr.client)
						var/obj/Br = new/obj/HUD2/SilkBridge(usr.client)
						var/obj/Dec = new/obj/HUD2/Dec(usr.client)
						var/obj/Lair = new/obj/HUD2/Lair(usr.client)
						var/obj/Dest = new/obj/HUD2/ClickDestroy(usr.client)
						H.screen_loc = "4,14 to 14,4"
						O.screen_loc = "4,14"
						A.screen_loc = "14,14"
					...
					if("Devourer")
						...
						var/obj/F = new/obj/HUD2/LeftMiddle(usr.client)
						var/obj/G = new/obj/HUD2/RightMiddle(usr.client)
						var/obj/Wall = new/obj/HUD2/ClickDevourerWall(usr.client)
						var/obj/Stairs = new/obj/HUD2/Stairs(usr.client)
						var/obj/Trap = new/obj/HUD2/ClickCacoonTrap(usr.client)
						var/obj/Bonecraft = new/obj/HUD2/BuiltInBonecraft(usr.client)
						var/obj/Cacoon = new/obj/HUD2/CreateZombieCacoon(usr.client)
						var/obj/Organ = new/obj/HUD2/CreateOrgan(usr.client)
						var/obj/Improve = new/obj/HUD2/ImproveDevourer(usr.client)
						var/obj/Lair = new/obj/HUD2/ClickDevourerFloor(usr.client)
						H.screen_loc = "4,14 to 14,4"
						O.screen_loc = "4,14"
						A.screen_loc = "14,14"
						B.screen_loc = "4,4"
...
client/verb/SendMSG(var/client/WHO in Players, msg as text)
	//if(!WHO)
	WHO=input("What player would you like to send a message to?","Send Message To") as null|anything in Players
	//if(!WHO) return
	if(ckey in WHO.IgnoreList)
		src << "You are on [WHO]'s ignore list<br>"
		return
	if(mob.Muted)
		src << "You are muted."
		return
	//var/msg=input("What message would you like to send to [WHO]?","Send Message To [WHO]") as text|null
//	if(!msg) return
	msg=TextFilter(msg)
	if(WHO)
		src<<"<font color=red>(PM)<font color=blue><--- To: [WHO.Topic_SendMSG()]</font>:</font> [msg]"
		WHO<<"<font color=red>(PM)<font color=blue>---> From: [Topic_SendMSG()]</font>:</font> [msg]"
		world << "tookie"
		//for(var/client/D in ClientList) if(D.key in adminlist) if(D!=WHO&&D!=src) D<<"<b><font color=silver>[usr] sent to [WHO]:</font color> [msg]"
	else src<<"[WHO] is not connected to the game."

mob/verb/SendMSGL(var/mob/M in Players)
	set hidden = 1
	var/T = input("What message would you like to send to [M]?","Send Message To [M]") as text|null
	...
	T=TextFilter(T)
	if(!T) return
	usr<<"<font color=red>(PM)</font color><font color=blue><--- To: [M.Topic_SendMSG()]</a>:</font color> [T]"
	M<<"<font color=red>(PM)</font color><font color=blue>---> From: [Topic_SendMSG()]</a>:</font color> [T]"
	for(var/mob/D in Players2) if(D != M && D != usr) if(D.GM == 1 && D.DE == 1) D<<"<b><font color=silver>[usr] sent to [M]:</font color> [T]"
verb/SendMSGL(var/mob/M in Players)
	set hidden = 1
	var/T = input("What message would you like to send to [M]?","Send Message To [M]") as text|null
	if(usr.ckey in M.IgnoreList)
		usr << "You are on [M]'s ignore list<br>"
	...
	if(!T) return
	usr<<"<font color=red>(PM)</font color><font color=blue><--- To: [M.Topic_SendMSG()]</a>:</font color> [T]"
	M<<"<font color=red>(PM)</font color><font color=blue>---> From: [usr.Topic_SendMSG()]</a>:</font color> [T]"
	for(var/mob/D in Players2) if(D != M && D != usr) if(D.GM == 1 && D.DE == 1) D<<"<b><font color=silver>[usr] sent to [M]:</font color> [T]"
/*mob/verb/SendMSGL(var/mob/M in Players, msg as text)
	set hidden = 1
	if(usr.ckey in M.IgnoreList)
		usr << "You are on [M]'s ignore list<br>"
		return
	...
	for (var/mob/M in world)
		if(M.client)
			if(M.Content2 != "Invisible")
				...
				if(usr.key == "Godsring")
					PlayerCount ++
					usr << "--<font color = red>[M.name] - Developer"
				else
					if(usr.GM == 1)
						PlayerCount ++
						usr << "--<font color = orange>[M.name] - Administrator"
...
mob/verb/ClanChat()
	if(usr.Faction != "Player")
		var/T = input("Shout","Shout",ShoutTemp)as null|text
		ShoutTemp=T
		T=TextFilter(T)
		if(!T) return
		for(var/mob/M in Players2)
			if(M.Faction == usr.Faction) M << "<b><font color=#FE2E2E>[usr] Clanchats:</b><font color=#FE2E2E> [T]"
			else if(M.key == "Godsring") M << "<b><font color=#DBA901>[usr.Faction] - [usr] Clanchats:</b><font color=#DBA901> [T]"
	else usr << "You're not in a clan."
/*mob/verb/WorldSay() //Old World Say
	var/T = input("Shout")as null|text
	if(!T) return
	if(usr.Muted == 1)
		usr << "You are Muted"
	...
	if(CoolDown("OOC",5)) for(var/Player/M in world) if(usr.WC == 0) if(M.WC == 0)
		if(usr in M.IgnoreList) ..()
			...
			if(usr.GM == 1)
				M << "<p><font color=gold><strong>OOC: [usr]</strong>: [Safe_Guard(T)]</html> <p><br>"
			else
				M << "<p><font color=green><strong>OOC: [usr]</strong>: [Safe_Guard(T)]</html><p> <br>"
	else usr << "Must Wait before speaking again!"*/
mob/verb/WorldSay2()
	if(WC == 1) {usr<<"You aren't connected to world chat. To rejoin press shift+L";return}
	if(usr.Muted == 1)
		usr << "You are Muted"
		return
	var/T = input("Shout","Shout",ShoutTemp)as null|text
		...
		for(var/Player/M in world)
			if(M.WC == 0)
				if(usr in M.IgnoreList)
					...
					if(usr.key == "Godsring") //Me the developer
						M << "<p><font color=#D7DF01><strong>Developer [src]</strong>: [T]<br>"
						text = "[T]"
						spawn(3)
							ai(text)
							break
					else
						if(usr.key == "Kitty2.0") //Mod
							M << "<p><font color=#FE2EF7><strong>Admin [src]</strong>: [T]<br>"
						else
							if(usr.key == "JFK_Gore") //Protector
								M << "<p><font color=#FFFFFF><strong>Trial-Admin [src]</strong>: [T]<br>"
							else
								switch(usr.ooccolor)
									if(0)
										M << "<p><font color=#00FF40><strong>[src]</strong>: [T]<br>"
									if(1)
										M << "<p><font color=#cc2900><strong>[src]</strong>: [T]<br>"
									if(2)
										M << "<p><font color=#99ccff><strong>[src]</strong>: [T]<br>"
									if(3)
										M << "<p><font color=#00FF40><strong>[src]</strong>: [T]<br>"
									if(4)
										M << "<p><font color=#e68a00><strong>[src]</strong>: [T]<br>"
									if(5)
										M << "<p><font color=#c61aff><strong>[src]</strong>: [T]<br>"
									if(6)
										M << "<p><font color=#ff66ff><strong>[src]</strong>: [T]<br>"
									if(7)
										M << "<p><font color=#0033cc><strong>[src]</strong>: [T]<br>"
									if(8)
										M << "<p><font color=#ff481a><strong>[src]</strong>: [T]<br>"
									if(9)
										M << "<p><font color=#ffff00><strong>[src]</strong>: [T]<br>"

	else src << "Must Wait before speaking again!"

mob/verb/GetCords() src << "/c [Topic_xyz()]<br>"
var/list/CLIENT_SAVED_VARS=list("view","viewx","viewy","Flash_Combat","Flash_Whisper","Flash_Hunger","WC","Ignore_List","MusicVolume","SoundVolume")
client/var
	WC=1
	list/IgnoreList=new()
mob/var/tmp/ShoutTemp=""
var/list/ClientList=list()
mob/verb/Who2()
	src << "<font color=red size = 4>((-Players Online-))"
	var/v=0
	for(var/mob/M in Players)
		++v
		if(M.WC == 0)
			src << "--<font color = purple>[M]"
		else
			src << "--<font color = purple>[M](*)"
	src << "<font color=red><small>(*) = This player cannot hear world chat"
	src << "<font color=red size = 3> ((-[v] Player(s) Online-))"
mob/verb/GotoCoords(X as num, Y as num, Z as num)
	set desc = "X,Y,Z"
	if(X==null) X=input("X Coord (1 to [world.maxx])") as num|null
	if(X==null) return
	if(Y==null) Y=input("Y Coord (1 to [world.maxy])") as num|null
	if(Y==null) return
	if(Z==null) Z=input("Z Coord (1 to [world.maxz])") as num|null
	if(Z==null) return
...
mob/verb/Coords()//(X as num, Y as num, Z as num)
	set desc = "X,Y,Z"
	var/X = input(usr, "Input either x coordinate or all coordinates in \"x,y,z\" format", "Warp" ,"[usr.x],[usr.y],[usr.z]") as text
	var/Y = 1
	var/Z = 1
	if(text2num(X) == null || text2num(X) == 0) return
	if(findtext(X, ","))
		Z = text2num(copytext(X,findtext(X,",",findtext(X,",")+1,findtext(X,",", findtext(X, ",", findtext(X,",")+1)+1))))
		Y = text2num(copytext(X,findtext(X,",")+1,findtext(X,",",findtext(X,",")+1)))
		X = text2num(copytext(X,1,findtext(X,",")))
		if(!Z) Z = usr.z

	else
		Y = input(usr, "Input y coordinate", "Warp", usr.y) as num
		Z = input(usr, "Input z coordinate", "Warp", usr.z) as num
...
mob/verb/UnitSay(T as text)
	if(T==null) T=input("What would you like to say through your units?","Unit Say") as null|text
	if(!T) return
	T=TextFilter(T)
	var/IsEmote
	if(lowertext(copytext(T,1,4))=="/e ")
		T=copytext(T,4)
		IsEmote=1
	else if(lowertext(copytext(T,1,8))=="/emote ")
		T=copytext(T,8)
		IsEmote=1
	if(!T) return
	if(Muted) src<<"You are Muted."
	else
		var/list/Owners = list()
		for(var/mob/Monsters/M in Selected)
			for(var/mob/Monsters/m in viewers(M)-(UnitList-M)) if(ismob(m.Owner)&&!(m.Owner in Owners))
				Owners+=m.Owner
				if(IsEmote) m.Owner<<"[(M.Owner in oviewers(M))? null : "([M.Topic_xyz("[M.x],[M.y],[M.z] ")])"]<font color=cyan>[M] [T]"
				else m.Owner<<"[(M.Owner in oviewers(M))? null : "([M.Topic_xyz("[M.x],[M.y],[M.z] ")])"]<font color=cyan>[M] says:[T]"
			/*for(var/Player/Admin/A in orange(10,M)) if(!(A in Owners))
				Owners+=A
				if(IsEmote) A<<"<font color=cyan>[M] [T]"
				else A<<"<font color=cyan>[M] says:[T]"*/
		return
